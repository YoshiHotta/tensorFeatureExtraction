function [T,Uinit] = tucker_als_feature(X,R,varargin)
%TUCKER_ALS Higher-order orthogonal iteration for feature extraction.
% This function is originally written by Sandia Corporation and modified for feature extraction by Yoshihito Hotta .
%
...
if iscell(init)
    ...
    if strcmp(init,'random')
        Uinit = cell(N,1);
        for n = dimorder(2:end-1)
            Uinit{n} = rand(size(X,n),R(n));
        end
        Uinit{dimorder(end)} = speye(size(X, dimorder(end)));
